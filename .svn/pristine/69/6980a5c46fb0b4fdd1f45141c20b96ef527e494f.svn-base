...
@section('content')
    ...
    <p class="bg-info">
        ...
        <span style="margin-right:20px;">業務員：{{$ProjectData->salesNodeName . '_' . $ProjectData->salesName}}</span>   
    </p>
    <!--tool bar-->
        ...
        <tbody>
            ...
            @foreach($ProductList as $list)
                <tr>
                    ...
                    <td><input type="button" class="btn btn-sm btn-danger" value="刪除" onclick="DoDelete('{{$ProjectData->ID}}', '{{$list->ID}}')"></td>
                </tr>
            @endforeach
        </tbody>
    </table>
